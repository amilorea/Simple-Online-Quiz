<?php
	...
	try {
		...
		//	Query
		$query = "SELECT ID, contestname, teacher, created, COUNT( question.questionID ) as count, SUM(question.point) as total FROM `contest` LEFT OUTER JOIN `question` ON question.`contestID` = contest.ID WHERE contestname LIKE '%".$searchName."%' GROUP BY contest.ID";
		$return['query'] = $query;
